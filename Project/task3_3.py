...
def adapt_weights(old_weights, old_input, old_output, new_input, new_output):
    ...
    # Inicializar novos valores para fc1 (Xavier)
    ...
    # Inicializar novos valores para fc2 (Xavier)
    ...
    while attempts < max_attempts:
        try:
            ...
            if actual_input_size != input_size or actual_output_size != output_size:
                try:
                    #print(f"[Tentativa {attempts + 1}] Adaptando pesos de {input_size_atual}x{output_size_atual} para {input_size}x{actual_size}")
            ...
            # Garantir que o fitness seja finito
            ...
            print(f"[Tentativa {attempts + 1}] Erro durante a avaliação: {e}")
            if 'env' in locals():
                env.close()
            attempts += 1
...
# ----- MODIFICAÇÃO NA FUNÇÃO PRINCIPAL DE COEVOLUÇÃO -----
...
def run_coevolution(seed, num_generations=NUM_GENERATIONS, eval_full_every=10, samples_per_gen=100):
    ...
    accumulated_fitness = fitness_matrix.copy()  # Inicializar matriz acumulada
    ...
    for gen in range(1, num_generations + 1):  # Começa da geração 1
        ...
        print(f"Matriz de fitness (valores acumulados) para a geração {gen}:")
        ...
        # Guardar cópias das populações atuais antes da evolução
        old_structure_population = [s.copy() for s in structure_population]
        old_controller_population = [c.copy() for c in controller_population]
        ...
        # 13. Salvar dados da geração usando a matriz acumulada
        save_generation_data(
            generation=gen,
            structures=structure_population,
            controllers=controller_population,
    ...
    utils.create_gif_pair(best_structure, best_controller, filename=f"COEVOLUCIONARY_GA_CMAES_{SCENARIO}_seed{seed}.gif",duration=0.066,scenario=SCENARIO,steps=500,
...
def main():
    ...
    if choice == '1':
        # Configurar a seed
        print("\nConfigurando a seed...")
        RUN_SEED = 40  # Use a seed global ou defina uma nova
        np.random.seed(RUN_SEED)
        random.seed(RUN_SEED)
        torch.manual_seed(RUN_SEED)
